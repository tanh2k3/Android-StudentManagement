...
class StudentAdapter(val students: MutableList<StudentModel>): RecyclerView.Adapter<StudentAdapter.StudentViewHolder>() {
    ...
    override fun onBindViewHolder(holder: StudentViewHolder, position: Int) {
        ...
        holder.imageEdit.setOnClickListener {
            (holder.itemView.context as MainActivity).showEditStudentDialog(students, position, this)
        }

        holder.imageRemove.setOnClickListener {
            (holder.itemView.context as MainActivity).showDeleteConfirmation(students, position, this)
        }
